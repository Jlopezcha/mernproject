...
app.get("/user/:id", function (request, response) {
  ...
  User.findById(id,{__v:0, login_name:0, password: 0})
      .then((user) => {
        ...
        //add this to an array with user and then photos 
        //response.end(JSON.stringify(user));
        photoArray[0] = JSON.stringify(user);
        Photo.aggregate([
...
"comments.user.__v": 0 } }
            ...
            .then((photos) => {
              ...
              //find most recent photo logic
              const recentPhoto = JSON.parse(JSON.stringify(photos));
              let photoDates = [];
              for(let i = 0; i < recentPhoto.length; i++){
                photoDates[i] = recentPhoto[i].date_time;

              }

              let sortedDate = photoDates.sort()[photoDates.length - 1];

              let mostRecentPhoto = recentPhoto.find(({date_time}) => date_time === sortedDate);

              //find most commented photo logic 
              let commentArray = [];
              for(let n = 0; n < recentPhoto.length; n++){
                commentArray[n] = recentPhoto[n].comments.length;

              }

              let most_commentedPhoto = recentPhoto.find(({comments}) => comments.length === Math.max(...commentArray));
              

              photoArray[1] = JSON.stringify(mostRecentPhoto);
              photoArray[2] = JSON.stringify(most_commentedPhoto);
                  
              response.end(JSON.stringify(photoArray));
            }).catch((err) => {
